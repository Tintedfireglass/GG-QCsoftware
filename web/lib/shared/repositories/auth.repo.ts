...
export async function findLicenseKeyByKey(tx: Tx, key: string): Promise<LicenseKeyRow | null> {
    const rows = await tx.select({
        ...
        product_scope: licenseKeys.productScope,
        platform_caps: licenseKeys.platformCaps,
...
/** Count distinct device activations for a key on a given platform (for per-platform caps). */
export async function countActivationsByPlatform(tx: Tx, licenseKeyId: number, platform: string): Promise<number> {
    const rows = await tx.select({ n: sql<number>`COUNT(*)::int` }).from(licenseKeyActivations)
        .where(and(
            eq(licenseKeyActivations.licenseKeyId, licenseKeyId),
            eq(licenseKeyActivations.platform, platform),
        ));
    return rows[0]?.n ?? 0;
}
