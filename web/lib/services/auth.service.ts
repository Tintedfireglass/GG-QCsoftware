...
import { buildFingerprint, isValidEmail } from '@/lib/domain/fingerprint';
import { ValidationError, UnauthorizedError, ForbiddenError, ConflictError, AppError } from '@/lib/http/errors';
import { LoginInput, RegisterInput, LicenseActivateInput, TrialInput } from '@/lib/domain/schemas/auth';
import { UserRole } from '@/lib/types';
import * as repo from '@/lib/repositories/auth.repo';
    ...
    return db.transaction(async (tx) => {
        ...
        if (!(await repo.isMachineActivated(tx, license.id, serial))) {
            ...
            if (currentUses >= license.max_uses) {
                throw new UnauthorizedError('This license key has reached its maximum machine activation limit');
            }
            await repo.insertActivation(tx, license.id, serial);
